 ...
 *      비디오 확장자 파일에는 TMDB 정보 찾기 아이콘을 붙여, 경로명의 마지막 폴더(titleKo)와
 *      파일명에서 추측한 영어제목/연도(titleEn/year)로 TmdbSearchPopup을 띄운다.
...
import { FileVideo, ScanSearch } from 'lucide-react'
import { apiClient } from '@/lib/apiClient'
import { formatFileSize, formatDate } from '@/lib/utils'
import { Button } from '@/shared/components/ui/button'
import TmdbSearchPopup from '@/domain/tmdb/components/TmdbSearchPopup'
import { isVideoExtension, guessTitleYearFromFileName, lastPathSegment } from '@/domain/movie/utils/movieNameGuess'
...
export default function HddSearchResults({ keyword }: HddSearchResultsProps) {
  ...
  const [tmdbTarget, setTmdbTarget] = useState<TmdbTarget | null>(null)
  ...
  return (
    <>
          ...
          {isLoading ? (
            ...
            <table className="w-full table-fixed text-sm">
              <thead className="bg-gray-50 border-b border-gray-100 sticky top-0">
                <tr>
                  <th className="text-left p-3 text-xs font-semibold text-gray-500 uppercase w-3/12">이름</th>
                  ...
                  <th className="text-center p-3 text-xs font-semibold text-gray-500 uppercase w-1/12">TMDB</th>
                </tr>
              </thead>
              <tbody>
                {files.map((f) => {
                  ...
                  return (
                    <tr key={f.id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="p-2 text-center">
                        {isVideoExtension(f.extension) && (
                          <Button
                            size="icon-sm"
                            variant="action"
                            title="TMDB 정보 찾기"
                            onClick={() => {
                              const guess = guessTitleYearFromFileName(f.name)
                              setTmdbTarget({ titleKo: lastPathSegment(dir) || guess.titleEn, ...guess })
                            }}
                          >
      ...
      <TmdbSearchPopup
        ...
        titleKo={tmdbTarget?.titleKo}
        titleEn={tmdbTarget?.titleEn}
        year={tmdbTarget?.year}
      />
    </>
  )
}
