 ...
 *      비디오 확장자 파일에는 TMDB 정보 찾기 아이콘을 붙여, 선택된 폴더명(titleKo)과
...
import { isVideoExtension, guessTitleYearFromFileName } from '@/domain/movie/utils/movieNameGuess'
          ...
          {isLoading ? (
            ...
            <table className="w-full table-fixed text-sm">
              <thead className="bg-gray-50 border-b border-gray-100 sticky top-0">
                <tr>
                  <th className="text-left p-3 text-xs font-semibold text-gray-500 uppercase w-6/12">이름</th>
                    ...
                    <td className="p-2 text-center">
                      {isVideoExtension(f.extension) && (
                        <Button
                          ...
                          variant="action"
                          title="TMDB 정보 찾기"
                          onClick={() => setTmdbTarget({ titleKo: selected.name, ...guessTitleYearFromFileName(f.name) })}
                        >
